...
void Trie<ValueType>::removeTree(Node * p) {
	if (p == nullptr) {
		...
		for (int i = 0; i < size; i++) {
			if (hasValue(p->child[i]) {
				delete p->child[i]->value;
			}
...
template <typename ValueType>
inline
//if exactMatchOnly is true, return a vector containing all values associated w/ key string. No required ordering of returned values if no values found, return an empty vector.
...
std::vector<ValueType> Trie<ValueType>::find(const std::string& key, bool exactMatchOnly) const {
	...
	trace(m_root, key, exactMatchOnly, v);
	return v;
...
void Trie<ValueType>::trace(Node * root, std::string& key, bool exactMatchOnly, std::vector<ValueType>& result) {
	...
	for (vector<Node*>iterator::it = root->child.begin(); it != root->child.end(); it++) {
		if ((*it->label == key[0])) {
			...
			trace(*it, key.substr(1, key.size() - 1, exactMatchOnly, result));
		}
